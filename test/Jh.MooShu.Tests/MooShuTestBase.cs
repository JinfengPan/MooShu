...
namespace Jh.MooShu.Tests
{
    public abstract class MooShuTestBase : AbpIntegratedTestBase<MooShuTestModule>
    {
        ...
        protected void UsingDbContext(Action<MooShuDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MooShuDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MooShuDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MooShuDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MooShuDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MooShuDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MooShuDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MooShuDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MooShuDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MooShuDbContext, Task<T>> func)
        {
            T result;
